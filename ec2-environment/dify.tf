...
resource "aws_instance" "dify" {
  ...
  user_data = <<-EOF
    ...
    cd /dify/docker
    sudo git switch -d 0.15.3
    sudo cp .env.example .env

    # 環境変数修正
    sed -i \
      -e 's/^DB_USERNAME=.*/DB_USERNAME=${aws_rds_cluster.aurora_postgres.master_username}/' \
      -e 's/^DB_PASSWORD=.*/DB_PASSWORD=${random_password.aurora_master.result}/' \
      -e 's/^DB_HOST=.*/DB_HOST=${aws_rds_cluster.aurora_postgres.endpoint}/' \
      -e 's/^DB_DATABASE=.*/DB_DATABASE=${aws_rds_cluster.aurora_postgres.database_name}/' \
      -e 's/^REDIS_HOST=.*/REDIS_HOST=${aws_elasticache_replication_group.redis.primary_endpoint_address}/' \
      -e 's/^REDIS_PASSWORD=.*/REDIS_PASSWORD=${random_password.redis.result}/' \
      -e 's/^REDIS_USE_SSL=.*/REDIS_USE_SSL=true/' \
      -e 's/^CELERY_BROKER_URL=.*/CELERY_BROKER_URL=redis://:${random_password.redis.result}${aws_elasticache_replication_group.redis.primary_endpoint_address}:6379/1|' \
    .env

    sudo docker compose up -d

    # db コンテナと redis コンテナを停止
    sudo docker container stop docker-db-1
    sudo docker container stop docker-redis-1
  EOF
}
